...
class QFileSaveDialog(QFileDialog):
    ...
    def updateButtonTranslations(self, path=None):
        ...
        for button in buttons:
            obj_name = button.objectName()
            if 'save' in obj_name.lower() or 'accept' in obj_name.lower() or 'ok' in obj_name.lower():
                save_button = button
            elif 'cancel' in obj_name.lower() or 'reject' in obj_name.lower():
                cancel_button = button
        
        # If not found by object name, try by text
        if not save_button:
            for button in buttons:
                if button.text().lower() in ['save', '&save']:
                    save_button = button
                    break
