...
class QFileSaveDialog(QFileDialog):
    ...
    def translateDialogTexts(self):
        ...
        print("Translating save dialog texts...")
        
        # Translate labels - print all labels to debug
        for child in self.findChildren(QLabel):
            text = child.text().strip()
            print(f"Label found in save dialog: '{text}'")
        ...
        # Also try to find the file name line edit
        for lineEdit in self.findChildren(QLineEdit):
            print(f"LineEdit found: {lineEdit.objectName()}")
            if 'name' in lineEdit.objectName().lower() or lineEdit.objectName() == 'fileNameEdit':
                # Set placeholder text
                lineEdit.setPlaceholderText(i18n.get('dialogs.file_name', 'File name'))
                print(f"  - Set placeholder to: {i18n.get('dialogs.file_name', 'File name')}")
    ...
    def updateButtonTranslations(self, path=None):
        ...
        print("Updating save dialog button translations...")
        print(f"Save text: {save_text}, Cancel text: {cancel_text}")
        ...
        for button in buttons:
            print(f"Button found: '{button.text()}' (objectName: {button.objectName()})")
        ...
        for button in buttons:
            ...
            if obj_name and ('save' in obj_name.lower() or 'accept' in obj_name.lower() or 'ok' in obj_name.lower()):
                save_button = button
                print(f"  - Found save button by objectName: {obj_name}")
            elif obj_name and ('cancel' in obj_name.lower() or 'reject' in obj_name.lower()):
                cancel_button = button
                print(f"  - Found cancel button by objectName: {obj_name}")
        ...
        if not save_button:
            for button in buttons:
                ...
                if text in ['save', '&save']:
                    save_button = button
                    print(f"  - Found save button by text: {button.text()}")
                    break
        ...
        if not cancel_button:
            for button in buttons:
                ...
                if text in ['cancel', '&cancel']:
                    cancel_button = button
                    print(f"  - Found cancel button by text: {button.text()}")
                    break
        
        # Apply translations if buttons were found
        if save_button:
            save_button.setText(save_text)
            print(f"  - Translated save button to: {save_text}")
        
        if cancel_button:
            cancel_button.setText(cancel_text)
            print(f"  - Translated cancel button to: {cancel_text}")
        ...
        for button in buttons:
            ...
            if text in ['save', '&save']:
                ...
                print(f"  - Force translated button '{text}' to: {save_text}")
            elif text in ['cancel', '&cancel']:
                button.setText(cancel_text)
                print(f"  - Force translated button '{text}' to: {cancel_text}")
        ...
        if buttonBox:
            for button in buttonBox.buttons():
                ...
                if role == 0:  # AcceptRole
                    button.setText(save_text)
                    print(f"  - Translated QDialogButtonBox AcceptRole button to: {save_text}")
                elif role == 1:  # RejectRole
                    button.setText(cancel_text)
                    print(f"  - Translated QDialogButtonBox RejectRole button to: {cancel_text}")
